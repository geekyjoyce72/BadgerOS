...
static uint16_t offsets[] = {
    ...
    0x0074, 0x0078, 0x007C, 0x0080, 0x0084, 0x008C, 0x0090, 0x0094,
    0x0098, 0x009C, 0x00A0, 0x00A4, 0x00A8, 0x00AC, 0x00B0, 0x00B4,
    0x00B8, 0x00BC, 0x00C0, 0x00C4, 0x00C8, 0x00CC, 0x00D0, 0x00D4,
    0x00D8, 0x00DC, 0x00E0, 0x00E4, 0x00E8, 0x00EC, 0x00F0, 0x00F4,
    0x00F8, 0x00FC, 0x0100, 0x0104, 0x0108, 0x010C, 0x0110, 0x0114,
    0x0118, 0x011C, 0x0120, 0x0124, 0x0128, 0x012C, 0x0130
};
...
void intmtx_init() {
    WRITE_REG(INTPRI_CORE0_CPU_INT_ENABLE_REG, 0xfffffffe);
    for (size_t i = 0; i < sizeof(offsets) / sizeof(uint16_t); i++) {
        WRITE_REG(INTMTX_BASE + offsets[i], 0);
    }
    for (size_t i = 0; i < 32; i++) {
        WRITE_REG(INTPRI_CORE0_CPU_INT_PRI_N_REG(i), 4);
    }
