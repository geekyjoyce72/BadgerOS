...
bool format_str_output(format_str_type_t type, format_str_spec_t spec, bool is_signed, bool lowercase, long long value, format_str_cb_t callback, void *cookie) {
...
bool format_str_va(char const *msg, size_t length, format_str_cb_t callback, void *cookie, va_list vararg) {
    ...
    while (i < length) {
        ...
        if (msg[i] == '%' && i+1 < length && msg[i+1] == '%') {
            // A literal '%'.
            if (!callback("%", 1, cookie)) return false;
            i += 2;
            ...
            if (mult == FMT_MULT_ARR && spec == FMT_SPEC_CHAR && type == FMT_TYPE_CHAR && !joiner) {
                ...
                if (!callback(cstr, length, cookie)) return false;
                ...
                char const *cstr = va_arg(vararg, void const *);
                if (!callback(cstr, cstr_length(cstr), cookie)) return false;
                ...
                switch (type) {
                    case FMT_TYPE_CHAR:
                        if (is_signed) value = (signed char) va_arg(vararg, int);
                        else value = (unsigned char) va_arg(vararg, unsigned int);
                        break;
                        
                    case FMT_TYPE_SHORT:
                        if (is_signed) value = (short) va_arg(vararg, int);
                        else value = (unsigned short) va_arg(vararg, unsigned int);
                        break;
                        
                    case FMT_TYPE_INT:
                        if (is_signed) value = va_arg(vararg, int);
                        else value = va_arg(vararg, unsigned int);
                        break;
                        
                    case FMT_TYPE_LONG:
                        if (is_signed) value = va_arg(vararg, long);
                        else value = va_arg(vararg, unsigned long);
                        break;
                        
                    case FMT_TYPE_LLONG:
                        if (is_signed) value = va_arg(vararg, long long);
                        else value = va_arg(vararg, unsigned long long);
                        break;
                }
                if (!format_str_output(
                    type, spec, is_signed, lowercase,
            ...
            if (!callback(&msg[start], i-start, cookie)) return false;
        }
    }
